...
public class Digraph<V> {
    ...
    public static Boolean route(Digraph<Integer> dg, Integer start, Integer end){
    	
    	List<Integer> q = new ArrayList<Integer>();
    	q.add(start);
    	while(!q.isEmpty()){
    		Integer n = q.remove(0);
    		
    		if(n!=null){
    			for(Integer i:dg.neighbors.get(n)){
    ...
    public static void main (String[] args) {
        ...
        System.out.println("The graph " + (graph.isDag()?"is":"is not") + " a dag");
        System.out.println("BFS distances starting from " + 2 + ": " + graph.bfsDistance(2));
        System.out.println(route(graph,0,3));
